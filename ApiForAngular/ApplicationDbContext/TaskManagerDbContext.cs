...
namespace ApiForAngular.ApplicationDbContext
{
    public class TaskManagerDbContext : IdentityDbContext<ApplicationUser, ApplicationROle, Guid>
    {
        ...
        public DbSet<TaskStatuses> TaskStatusesTbl { get; set; }
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<ClientLocations>().HasData(
                ...
                new ClientLocations() { ClientLocationID = 3, ClientLocationName = "New Jersy" },
                new ClientLocations() { ClientLocationID = 4, ClientLocationName = "New York" },
                new ClientLocations() { ClientLocationID = 5, ClientLocationName = "London" },
                new ClientLocations() { ClientLocationID = 6, ClientLocationName = "Tokyo" }
            );
          ...
          new TaskStatuses() { TaskStatusID = 1, TaskStatusName = "Holding" }, //Tasks that need to be documented still
          new TaskStatuses() { TaskStatusID = 2, TaskStatusName = "Prioritized" }, //Tasks that are placed in priority order; so need to start ASAP
          new TaskStatuses() { TaskStatusID = 3, TaskStatusName = "Started" }, //Tasks that are currently working
          new TaskStatuses() { TaskStatusID = 4, TaskStatusName = "Finished" }, //Tasks that are finished workng
          new TaskStatuses() { TaskStatusID = 5, TaskStatusName = "Reverted" } //Tasks that are reverted back, with comments or issues
       );
        }
